...
class BubbleLines extends Component {
	// // GET MAX & MIN X
  // getMinX() {
  ...
  render() {
    const { data, svgHeight, svgWidth, color } = this.props;
    let pathD = data.map((line, i) => {
      return (
        <line className='bubbleline' key={line.x1.toString() + " " + line.y1.toString() + " " + line.x2.toString() + " " + line.y2.toString() + " " }
         ...
         stroke={color} strokeWidth="10" strokeLinecap="square"
        /> 
      );
    });
